...
class AReLiTNoMem(nn.Module):
    ...
    def setup(self):
        self.arelit = AReLiT(n_layers=self.n_layers, d_model=self.d_model, d_head=self.d_head, d_ffc=self.d_ffc,
                             n_heads=self.n_heads, eta=self.eta, r=self.r, reset_on_terminate=self.reset_on_terminate)
        self.initial_memory = AReLiT.initialize_memory(
            self.n_layers, self.n_heads, self.d_head, self.eta, self.r)

    def __call__(self, inputs):
        terminations = jnp.zeros_like(inputs[:, 0])
        out, mem = self.arelit(inputs, terminations, self.initial_memory)
        return out
...
class ClassificationModel(nn.Module):
    ...
    def setup(self):
        self.encoder = AReLiTNoMem(n_layers=self.n_layers, d_model=self.d_model, d_head=self.d_head,
                                   d_ffc=self.d_ffc, n_heads=self.n_heads, eta=self.eta, r=self.r)
...
if __name__ == '__main__':
    ...
    model = AReLiT(n_layers=4, d_model=256, d_head=64,
                   d_ffc=512, n_heads=4, eta=4, r=4)
    inputs = jnp.ones((10, 256))
    terminations = jnp.zeros((10,))
    memory = AReLiT.initialize_memory(4, 4, 64, 4, 4)
    rng = jax.random.PRNGKey(0)
    params = model.init(rng, inputs, terminations, memory)
    out, new_memory = model.apply(params, inputs, terminations, memory)
    print(out.shape)

    # Test the AReLiTNoMem model
    model = AReLiTNoMem(n_layers=4, d_model=256, d_head=64,
                        d_ffc=512, n_heads=4, eta=4, r=4)
    inputs = jnp.ones((10, 256))
